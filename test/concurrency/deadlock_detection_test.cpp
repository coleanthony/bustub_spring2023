...
TEST(LockManagerDeadlockDetectionTest, BasicDeadlockDetectionTest1) {
  ...
  std::thread t0([&] {
    ...
    // This will block
    // LOG_DEBUG("[BasicDeadlockDetectionTest1] block thread 0");
    res = lock_mgr.LockRow(txn0, LockManager::LockMode::EXCLUSIVE, toid, rid1);
    EXPECT_EQ(true, res);
  ...
  std::thread t1([&] {
    ...
    // This will block
    // LOG_DEBUG("[BasicDeadlockDetectionTest1] block thread 1");
  ...
  std::thread t2([&] {
    ...
    // This will block
    // LOG_DEBUG("[BasicDeadlockDetectionTest1] block thread 2");
